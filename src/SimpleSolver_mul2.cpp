...
Vector3d SimpleSolver_mul2::locate_simple(const std::vector<LocatorData>& locators) {
    ...
    //std::cout << "Value column:\n" << value_column << std::endl;
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        subtraction_mat.block(row, i, id_size, 1) = Eigen::MatrixXd::Constant(id_size, 1, -1);
        subtraction_mat.block(row, i + 1, id_size, id_size) = Eigen::MatrixXd::Identity(id_size, id_size);
        row += id_size;
    }
